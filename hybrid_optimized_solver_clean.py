...
class HybridOptimizedSolver:
    ...
    def solve_captcha_with_fallback(self, image_path: str, audio_path: str = None) -> Dict[str, Any]:
        """
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            multimodal_text = self.multimodal_solver.solve_captcha_multimodal(image_path, audio_path)
            if multimodal_text and self._validate_captcha_format(multimodal_text):
                ...
                result['attempts'].append(('multimodal', multimodal_text, 'success'))
                return result
            else:
                result['attempts'].append(('multimodal', multimodal_text or 'null', 'failed'))
        ...
        if self.image_solver.is_available():
            ...
            if image_text and self._validate_captcha_format(image_text):
                ...
                result['attempts'].append(('image_only', image_text, 'success'))
                return result
            else:
                result['attempts'].append(('image_only', image_text or 'null', 'failed'))
        
        # Stratégie 3: Audio seul si disponible (dernier recours)
        if audio_path and os.path.exists(audio_path):
            print("🎧 Dernier recours: Audio seul...")
            
            audio_text = self.multimodal_solver.solve_captcha_audio_only(audio_path)
            if audio_text and self._validate_captcha_format(audio_text):
                ...
                result['attempts'].append(('audio_only', audio_text, 'success'))
                return result
            else:
                result['attempts'].append(('audio_only', audio_text or 'null', 'failed'))
...
def test_hybrid_solver():
    ...
    # Utiliser les derniers fichiers capturés
    import glob
